...
<div class="block">
<img src="project\photo.jpg" style="margin:-150px 50px -30px"/>
</div>
</div>
</div>
...
function live()
  {
    ...
    $dbname="data";
$conn=new mysqli($servername,$username,$password,$dbname);
$Danish=mysqli_query($conn,"SELECT * FROM `dbo`");
while ($win=mysqli_fetch_assoc($Danish))
{
  print $win['имя'].'<br>';
  print $win['комментарий'].'<br>';
...
?>
  ...
  <p>Ваше имя:<br><input type="text" name="play"></p>
     ...
     <textarea type="text" name="message" cols="60" rows="5" border="2"></textarea></p>
	 <p align="left">
     <div class="glow">
     <input type="submit" value="Отправить"  align="right" style="margin:0px 350px 0px"></p>
   </div>
   </form>
  <div class="d-flex">
 <img src="project\photo.jpg" width="175" height="175"/>
</div>
